...
#       menu_bar.insertMenu(lastAction, self.menu )
   ...
   def initActions(self):
      ...
      self.lvdb_action = QAction(cmd.getIcon(), cmd.getName(), self.iface.mainWindow())
      self.lvdb_action.setToolTip(cmd.getToolTipText())
      cmd.connectQAction(self.lvdb_action)

      # LVOH
      cmd = self.QadCommands.getCommandObj(QadMsg.translate("Command_list", "LVOH"))
      self.lvoh_action = QAction(cmd.getIcon(), cmd.getName(), self.iface.mainWindow())
      self.lvoh_action.setToolTip(cmd.getToolTipText())
      cmd.connectQAction(self.lvoh_action)

      # LVUG
      cmd = self.QadCommands.getCommandObj(QadMsg.translate("Command_list", "LVUG"))
   ...
   def createDrawMenu(self):
      ...
      # menu lv
      self.lvMenu = self.createLvMenu()
      drawMenu.addMenu(self.lvMenu)
   ...
   def createLVToolButton(self):
      lvToolButton = QToolButton(self.toolBar)
      lvToolButton.setPopupMode(QToolButton.MenuButtonPopup)
      lvToolButton.setMenu(self.lvMenu)
      lvToolButton.setDefaultAction(self.lvMenu.actions()[0]) # prima voce di menu
      lvToolButton.triggered.connect(self.lvToolButtonTriggered)
      return lvToolButton
   def lvToolButtonTriggered(self, action):
      self.lvToolButton.setDefaultAction(action)
   ...
   def runLVOHCommand(self):
      self.runCommandAbortingTheCurrent(QadMsg.translate("Command_list", "LVOH"))
